package main.java.homeworks.homework07.Person;
    ...
    public void addToBasket (Product product) {
        if (product instanceof DiscountProduct discountProduct) {
            if (LocalDate.now().isBefore(discountProduct.getDiscountDate())
                    && this.cashPerson > discountProduct.getDiscountedPrice()) {
                this.cashPerson -= discountProduct.getDiscountedPrice();
                basket.add(discountProduct);
            }
        } else if (this.cashPerson > product.getPriceProduct()) {
